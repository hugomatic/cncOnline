...
params = hugomatic.toolkit.Parameters('Rectangle', 'Squarely rectanglar', 
...
mode = modes[2]
params.addArgument(mode,  'dimensions mode', choices=modes, group='rectangle' ) 
x0 = 0.
params.addArgument(x0, 'x0 left', group='rectangle' ) 
y0 = 0.
params.addArgument(y0, 'y0 bottom', group='rectangle' ) 
x1 = 1.
params.addArgument(x1,  'x1 right', group='rectangle' ) 
y1 = 1.
params.addArgument(y1,  'y1 top', group='rectangle' )
    ...
    for j in range(ycount):
        for i in range (xcount):
            ...
            if mode == modes[0]: # tool outside
                hugomatic.code.rectangle_tool_outside(xleft, ybottom, xright, ytop, zsafe, zsurf,  tooldia, cuts)
            
            if mode == modes[1]: # tool inside
                hugomatic.code.rectangle_tool_inside(xleft, ybottom, xright, ytop, zsafe, zsurf,  tooldia, cuts)
            
            if mode == modes[2]: # tool on
                hugomatic.code.rectangle(xleft, ybottom, xright, ytop, zsafe, zsurf,  tooldia, cuts)                
